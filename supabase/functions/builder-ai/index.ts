...
- Si el usuario dice "agrega clientes", "crea un tab de facturas" → type=page_add
- Si el usuario dice "cambia el color", "modifica el titulo" → type=modification
- Si el usuario dice "agrega un cliente llamado Juan" o "muestrame los productos" → type=data_operation
...
EJEMPLOS:
Input: "crea un crm para mi empresa TechCo" → {"type":"generation","intent":"crm","businessName":"TechCo","sections":["dashboard","clientes","reportes"],"colorScheme":"modern","industry":"technology","confidence":0.98}
Input: "agrega un cliente llamado Juan con email juan@mail.com" → {"type":"data_operation","tableName":"clients","operation":"create","data":{"name":"Juan","email":"juan@mail.com"},"confidence":0.95}
Input: "muestrame los productos" → {"type":"data_operation","tableName":"products","operation":"read","confidence":0.90}
Input: "cambia el color a azul" → {"type":"modification","confidence":0.95}
Input: "hola que tal" → {"type":"conversational","conversationalResponse":"¡Hola! 👋 Soy DOKU AI...","confidence":0.99}
      ...
      try {
        ...
        if (smartResult.operation === "create" && smartResult.data) {
          ...
          if (tables) {
            ...
            return new Response(
              JSON.stringify({
                intent: "data_operation", confidence: smartResult.confidence, label: "Operación de datos",
                entities: { businessName: "", sections: [], colorScheme: "", industry: "" },
                html: "", conversationalResponse: `✅ Registro creado en **${smartResult.tableName}**:\n${dataKeys}\n\nLos datos ya están disponibles en tu sistema.`,
              }),
              { headers: { ...corsHeaders, "Content-Type": "application/json" } }
            );
          } else {
            return new Response(
              JSON.stringify({
                intent: "data_operation", confidence: smartResult.confidence, label: "Operación de datos",
                entities: { businessName: "", sections: [], colorScheme: "", industry: "" },
                html: "", conversationalResponse: `⚠️ La tabla **${smartResult.tableName}** no existe en este proyecto. Primero genera un sitio que incluya esa sección.`,
              }),
              { headers: { ...corsHeaders, "Content-Type": "application/json" } }
            );
          }
          ...
          if (tables) {
            ...
            let summary = `📊 **${smartResult.tableName}**: ${count} registros`;
